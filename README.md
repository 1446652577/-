...
在微信开发者工具 → 云开发 → 数据库中，创建以下5个集合。**创建时必须设置权限**，否则数据会被任意读写：
...
| `parse_jobs` | 扫描版 PDF 的异步解析任务 | **仅创建者可读写** |
...
5. **parse_jobs** - 扫描版 PDF 的异步解析任务（由 `parseDocument` 云函数写入）
...
- `cloudfunctions/getQuizQuestions`
